...
//        botoesDaTela[0].backgroundColor = UIColor.gray;
    ...
    @IBAction func clicouBotaoCodigo(_ sender: UIButton) {
        ...
        present(telaVerdeViewController, animated: true, completion: {
    ...
    @IBAction func buttonNavControllerPressed(_ sender: Any) {
        let storyboardMain = UIStoryboard(name: "Main", bundle: nil)
        if let telaVerdeViewController = storyboardMain.instantiateViewController(withIdentifier: "telaVerdeStoryboardID") as? TelaVerdeViewController {
...
extension TelaInicialViewController: TelaVerdeViewControllerDelegate {
